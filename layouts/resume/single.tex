...
% License : MIT
% https://www.overleaf.com/project/6555227ca601c30288768330
...
\usepackage[T2B,T1]{fontenc}                              % внутренняя T2A кодировка TeX
\usepackage{tempora}
\usepackage[english, russian, ukrainian]{babel}
\usepackage{cmap}
% \usepackage{cyrtimes}

\usepackage{lmodern} % або інший шрифт, який підтримує T2B
...
% \usepackage[english, russian, ukrainian]{babel} % provides culturally determined typographical rules for wide range of languages
%-----------------------------------------
...
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
  ...
  \textbf{\Huge  {{ .name }}  \vspace{2pt}} & % row = 1, col = 1
  {{ end }}
{{ with .Params.header.infos }}  
%   Location: Anycity, Anystate, Anycountry 
   \\ % row = 1, col = 2
{{ if .title }}{{ .title }}{{ end }}
  \href{ {{ .website }} }{\uline{ {{ .website }} }} $|$ % row = 2, col = 1
  {{ if .email }} Email: \href{mailto:  {{ .email }} }{\uline{ {{ .email }} }}{{ end }} $|$ % row = 2, col = 2
...
{{ range .sections }}
  \resumeQuadHeading{ {{ if .title }}{{ .title }}{{ end }} }{ {{ if .startDate }}{{ .startDate }}{{ end }} {{ if .endDate }}  --  {{ .endDate }}{{ end }} }
